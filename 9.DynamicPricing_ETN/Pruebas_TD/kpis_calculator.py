...
def calcular_kpis(df:pd.DataFrame, colYear, valYear, months=None) -> dict:
    kpis = {}
    ...
    ##################################################################
    # Suponiendo que tu columna de fecha se llama 'FECHA'
    # (si tiene otro nombre, cámbialo en el código)

    ayer = pd.Timestamp.now().normalize() - pd.Timedelta(days=1)
    if months:
        tres_meses_atras = ayer - pd.DateOffset(months=3)
        # Filtramos el dataframe principal
        df_filtrado = df[(df["FECHA_CORRIDA"] >= tres_meses_atras) & (df["FECHA_CORRIDA"] <= ayer)]
    else:
        # Filtramos el dataframe principal
        df_filtrado = df[ df["FECHA_CORRIDA"] <= ayer ]

    # --- Ocupación promedio por hora, día y origen
    kpis["ocupacion_promedio"] = (
        df_filtrado.groupby(["ORIGEN", "DESTINO", "CAPACIDAD_ASIENTOS_TRAMO", "NOMBRE_DIA_CORRIDA", "HORA_DECIMAL"])[["BOLETOS_VEND", "VENTA", "OCUPACION_TRAMO"]]
        .mean()
        .reset_index()
    )
    ...
    kpis["venta_anticipada"]['DIF_PROM_COSTO'] = kpis["venta_anticipada"]['COSTO_PROM_BOLETO'] / kpis["venta_anticipada"]['TARIFA_BASE_TRAMO']

    ##################################################################
    # --- Top asientos más vendidos por hora y día
    kpis["venta_x_asientos"] = (
        df_filtrado.groupby(["CAPACIDAD_ASIENTOS_TRAMO", "NUM_ASIENTO"]).agg(
    ...
    kpis["venta_x_asientos"]["TOTAL_PAX_X_CAP"] = kpis["venta_x_asientos"].groupby("CAPACIDAD_ASIENTOS_TRAMO")["PAX_X_DIA_PROM"].transform("sum")
    # Paso 2: calcular la proporción por asiento
    kpis["venta_x_asientos"]["PROP_ASIENTO"] = kpis["venta_x_asientos"]["PAX_X_DIA_PROM"] / kpis["venta_x_asientos"]["TOTAL_PAX_X_CAP"]
    ...
    kpis["ventas_por_origen"] = (
        df.groupby(["ORIGEN", "DESTINO"])[["BOLETOS_VEND", "VENTA"]].mean().reset_index()
    )
    ...
    df_prueba["RANGO_TARIFA"] = pd.cut(df_prueba["VENTA"], bins=bins, labels=labels, right=False, include_lowest=True)
